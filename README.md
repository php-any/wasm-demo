...
脚本会自动获取 `github.com/php-any/origami@main` 的最新代码，并将所有前端资源与 `main.wasm` 输出到 `docs/`（默认，适合 GitHub Pages），或可选输出到独立的 `dist/` 目录。

```bash
# 默认输出到 docs/（适合 GitHub Pages，构建前会自动拉取 origami 最新 main）
./build.sh
...
#   Branch: main
#   Folder: /docs
# 页面地址通常为：https://<your-account>.github.io/<repo>/
```
...
- `build.sh`：构建脚本，生成 `docs/` 或 `dist/`
- `serve.sh`：本地预览 `docs/` 或 `dist/`
- `snippets/`：示例代码片段（会被复制到目标目录的 `snippets/`）
- `dist/`：独立可部署目录（构建产物）
- `docs/`：默认构建输出目录（用于 GitHub Pages）
